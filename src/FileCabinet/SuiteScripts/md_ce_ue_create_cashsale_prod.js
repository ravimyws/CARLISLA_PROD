...
define(['N/record','N/runtime','N/render','N/email','N/search','N/query','N/https'], function(record,runtime,render,email,search,query,https) {
    ...
    function afterSubmit(context) {
        try{
            ...
            var response = https.requestRestlet({
              body: JSON.stringify({id:recId,api_command:"CREATE_CASHSALE"}),
              deploymentId: "customdeploy_md_ce_rs_iop",
              scriptId: "customscript_md_ce_rs_iop",
              headers: {'Content-Type':'application/json'},
              method: "POST",
            });
            var csId = parseInt(response.body);
            var role = runtime.getCurrentUser().role;
            log.audit({title:"role",details:role});
            log.audit({title:"csId",details:JSON.stringify(response)});
            ...
            var trandate = ifRec.getValue("trandate");
              ...
              if (userObj.id==104412||userObj.id==4||userObj.id==3||userObj.id==16) {//kia, vincent, brian brian
                    email.send({
    ...
    function handleCommissionsAndReturnHoldBack(cashSaleRecord) {
        ...
        try {
          log.error("handleCommissionsAndReturnHoldBack", cashSaleRecord.id);
            ...
            let custbody_ce_inv_comm_vb = transactionObj.getValue('custbody_ce_inv_comm_vb');
            if(custbody_ce_inv_comm_vb && custbody_ce_inv_comm_vb.length >0){
                log.error('Reason',`RELATED COMMISSION VENDOR BILL/CREDIT already exist`);
                return;
            }
            ...
            if (salesOrderType) {
              ...
              if (isAgency) {
                ...
                if (!salesOrderCommission && associateCommission > vendorIOPCommissionObj) {
              ...
              if (isAgency) {
                ...
                }else if(associateCommission > salesOrderCommission){
                  let associateOptions = {
                    tranId: transactionObj.id,
                    primaryCommission: salesOrderCommission,
                    vendorId: associate,
                    brand: brand,
                    seasonIop: seasonIop,
                  };
